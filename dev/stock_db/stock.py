...
class Stock:
	...
	def range_exists(self,symbol,sd,ed):
		...
		if s.exists_av == False and s.exists_quandl == False:
			stock_range =['cant scrape']
			print stock_range
		elif s.minDate == None and s.maxDate == None and s.exists_av == None:
			data_scrape = self.scrape_date_range(symbol,sd,ed)
			s = stock.query.filter_by(symbol=symbol).first()
			if s.minDate == None and s.maxDate == None:
				stock_range =['cant pull currently']
				print stock_range
			else:
				stock_range=self.return_range(symbol,sd,ed)
		else:
	...
	def scrape_date_range(self,symbol,start_date,end_date):
		...
		elif s.exists_quandl == False and (s.exists_av == True or s.exists_av == None):
